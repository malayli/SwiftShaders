...
fragment float4 gaussianBlurFragment(VertexOut fragmentIn [[stage_in]], texture2d<float, access::sample> customTexture [[texture(0)]], constant FragmentUniforms &uniforms [[buffer(0)]]) {
    ...
    // 9 tap filter
    sum += customTexture.sample(qsampler, float2(offset.x - 4.0*xPixel*blur, offset.y - 4.0*yPixel*blur)).rgb * 0.0162162162;
    sum += customTexture.sample(qsampler, float2(offset.x - 3.0*xPixel*blur, offset.y - 3.0*yPixel*blur)).rgb * 0.0540540541;
    sum += customTexture.sample(qsampler, float2(offset.x - 2.0*xPixel*blur, offset.y - 2.0*yPixel*blur)).rgb * 0.1216216216;
    sum += customTexture.sample(qsampler, float2(offset.x - 1.0*xPixel*blur, offset.y - 1.0*yPixel*blur)).rgb * 0.1945945946;
